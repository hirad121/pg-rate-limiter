...
- **The real-Postgres concurrency test** (`test_concurrent_hits_on_same_subject_produce_no_lost_updates`) was written specifically to reproduce the source app's own proven result (8 concurrent hits on one subject → `hit_count == 8`, no lost updates) — but **could not be run against a local Postgres instance before this repo's first push**: the only local Postgres available (native Windows, port 5432) has a superuser password this effort doesn't have access to, and reading the sibling app's real `.env` to find one was correctly refused rather than worked around. This test instead ran for the first time in CI (`.github/workflows/ci.yml`'s `postgres-integration` job, a GitHub Actions `postgres:` service container) on the first push — check that job's status badge/history before trusting this claim if you're reading this shortly after initial publish.
